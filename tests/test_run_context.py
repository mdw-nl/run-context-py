...
def test_input_uris_mapping_not_implemented(tmp_path, monkeypatch):
    in_path = tmp_path / "input.csv"
    out_path = tmp_path / "out.json"
    ctx_path = tmp_path / "run_context.json"
    _write_json(
        ctx_path,
        _valid_payload(str(in_path), str(out_path), name="mapping_demo"),
    )
    monkeypatch.setenv("RUN_CONTEXT_FILE", str(ctx_path))

    @run_context(input_uris={"dataset_path": {"role": "dataset"}})
    def mapping_demo():
        raise AssertionError("should not be reached")
